...
\draw [-implies,double equal sign distance] (U) --node [midway,above,rotate=0]{$\scalebox{1.0}{\lref{lemma:is_a}}$} (SC);
\pgfextractangle{\angle}{U}{RC}
\draw [-implies,double equal sign distance] (U) --node [midway,above,rotate=0]{$\scalebox{1.0}{\lref{lemma:is_a}}$} (RC);
\pgfextractangle{\angle}{SC}{E}
\draw [-implies,double equal sign distance] (SC) --node [midway,above,rotate=0]{$\scalebox{1.0}{\lref{lemma:is_a}}$} (E);
\pgfextractangle{\angle}{E}{RC}
\draw [-implies,double equal sign distance] (RC) --node [midway,above,rotate=0]{$\scalebox{1.0}{\lref{lemma:is_a}}$} (E);
\pgfextractangle{\angle}{TC}{U}
\draw [->, thick] (U) to [out=240,in=90] node [midway,right,rotate=0]{$\scalebox{1.0}{\lref{lem:OTtoC}}\quad\enspace$}(TC);
\draw [-, thick] (E) to [out=60,in=270] (5,2);
\pgfextractangle{\angle}{U}{M}
\draw [->, thick] (5,2) to [out=90,in=300] node [pos=-0.05,left,rotate=0]{$\quad\scalebox{1.0}{\lref{lem:EtoU}}$} (U);
\draw [-, thick] (TC) to [out=0,in=300] (4.655,3);
\pgfextractangle{\angle}{SC}{E}
\draw [->, thick] (E) to [out=180,in=270] node [midway,below,rotate=0]{$\scalebox{1.0}{\lref{lem:EtoS}}$} (SC);
\pgfextractangle{\angle}{E}{RC}
\draw [->, thick] (E) to [out=0,in=270] node [midway,below,rotate=0]{$\scalebox{1.0}{\lref{lem:EtoR}}$} (RC);
\end{tikzpicture}
...
\paragraph{Implementation.} We implement several variants of out OT and OT extension protocols. Namely, we instantiate the OT framework from DDH and LWE using the Kyber key encapsulation protocol\cite{kyber} along with several of the extension protocols which realize $\OOT^\rec$ and $\OOT^\U$-security. In a LAN network the DDH and LWE OT protocols can perform 128 OTs in  28 and 24 milliseconds, respectively. In a more realistic WAN setting we observe that our one round DDH protocol is the fastest, requiring 110ms. The next fastest was the two round protocol of \cite{LC:ChoOrl15}, requiring 210ms in the WAN setting but only 18ms in the LAN setting. In addition to being faster than \cite{LC:ChoOrl15} in the WAN setting, our protocols achieve full simulation based security without performing addition rounds as \cite{LC:ChoOrl15} does. With regards to OT extension, our two round protocol achieving endemic security can processes 2 million OTs per second which is on par with an optimized version of \cite{C:KelOrsSch15,libOTe} in the LAN setting. In the WAN setting this protocol becomes 2.5 times faster than \cite{C:KelOrsSch15} (which achieves stronger security). Our protocol in the ideal cipher model can process 23 million OTs per second over the course of 5 rounds and achieve full uniform message security.
