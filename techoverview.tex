...
\node (M) at (5,2) {};
\pgfextractangle{\angle}{SC}{U}
\draw [-implies,double equal sign distance] (U) --node [midway,above,rotate=\angle]{$\scalebox{0.8}{\lemmaref{lemma:is_a}}$} (SC);
\pgfextractangle{\angle}{U}{RC}
\draw [-implies,double equal sign distance] (U) --node [midway,above,rotate=\angle]{$\scalebox{0.8}{\lemmaref{lemma:is_a}}$} (RC);
\pgfextractangle{\angle}{SC}{E}
\draw [-implies,double equal sign distance] (SC) --node [midway,above,rotate=\angle]{$\scalebox{0.8}{\lemmaref{lemma:is_a}}$} (E);
\pgfextractangle{\angle}{E}{RC}
\draw [-implies,double equal sign distance] (RC) --node [midway,above,rotate=\angle]{$\scalebox{0.8}{\lemmaref{lemma:is_a}}$} (E);
\pgfextractangle{\angle}{TC}{U}
\draw [->, thick] (U) to [out=240,in=90] node [midway,above,rotate=\angle]{$\scalebox{0.8}{\lemmaref{lem:OTtoC}}\quad\enspace$}(TC);
\draw [-, thick] (E) to [out=60,in=270] (5,2);
\pgfextractangle{\angle}{U}{M}
\draw [->, thick] (5,2) to [out=90,in=300] node [midway,above,rotate=\angle]{$\quad\scalebox{0.8}{\lemmaref{lem:EtoU}}$} (U);
\draw [-, thick] (TC) to [out=0,in=300] (4.655,3);
\pgfextractangle{\angle}{SC}{E}
\draw [->, thick] (E) to [out=180,in=270] node [midway,below,rotate=\angle]{$\scalebox{0.8}{\lemmaref{lem:EtoS}}$} (SC);
\pgfextractangle{\angle}{E}{RC}
\draw [->, thick] (E) to [out=0,in=270] node [midway,below,rotate=\angle]{$\scalebox{0.8}{\lemmaref{lem:EtoR}}$} (RC);
\end{tikzpicture}
...
In the actual protocol, the receiver can programm $r_0$, $r_1$ to a public key 
of his choice $b\in\bits$. He samples $r_{1-b}$ and sets $r_b=\pk\ominus\H(r_{1-b})$.
