...
\draw [->, thick] (5,2) to [out=90,in=300] node [pos=-0.05,left,rotate=0]{$\quad\scalebox{0.7}{\lref{lem:EtoU}}$} (U);
...
%	\draw [->, thick] (E) .. controls (7,0.75) .. (RC);
	...
	\myvspace{-0.3cm}
	\caption{
		The figure shows the implications of the base OT security (\definitionref{def:otSec}) when  $\Pi^\textsf{ext}$ from \figureref{fig:otExt} or a slight variation (dashed line) is applied. $\OOT^{*u}$ denotes $\OOT^*$ security where the receiver's selection is uniformly sampled by the functionality \definitionref{def:ot},\ref{def:got}.
	} 
