...
\begin{figure}
	...
	\node [anchor=east] (Base) at (3,5) {\underline{\normalsize Base OT Security}};
	\node [anchor=east](U) at (3,4) {Uniform Security $\OOT^{\U u}$};
	\node [anchor=east] (SCu) at (3,3) {Sender Chosen $\OOT^{\send u}$};
	\node [anchor=east](SC) at (3,2) {Sender Chosen $\OOT^{\send}$};
	\node [anchor=east] (RC) at (3,1) {Receiver Chosen  $\OOT^{\rec}$};
	\node [anchor=east] (E) at (3,0) {Endemic Security  $\OOT^{\E}$};
	
	
	\node [anchor=west](Ext) at (8.5,5) {\underline{\normalsize OT Extension Security}};
	\node [anchor=west](EU) at (8.5,4) {Uniform Security $\OOT^{\U}$};
	\node [anchor=west](ESC) at (8.5,3) {Sender Chosen $\OOT^{\send}$};
	\node [anchor=west](ERC) at (8.5,1) {Receiver Chosen  $\OOT^{\rec}$};
	\node [anchor=west](EE) at (8.5,0) {Endemic Security  $\OOT^{\E}$};
