...
\begin{figure}
	...
	\node (SCu) at (-0.1,3) {Sender Chosen $\OOT^{\send u}$};
	\node (SC) at (-0.1,2) {Sender Chosen $\OOT^{\send}$};
	\node (RC) at (-0.35,1) {Receiver Chosen  $\OOT^{\rec}$};
	...
	\node (ESC) at (12.13,3) {Sender Chosen $\OOT^{\send}$};
	\node (ERC) at (12.38,1) {Receiver Chosen  $\OOT^{\rec}$};
