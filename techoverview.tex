...
\begin{figure}
	...
	\node (EU)  [anchor=west] at (8.5,4) {Uniform Security $\OOT^{\U}$};
	\node (ESC) [anchor=west] at (8.5,3) {Sender Chosen $\OOT^{\send}$};
	\node (ERC) [anchor=west] at (8.5,1) {Receiver Chosen  $\OOT^{\rec}$};
	\node (EE)  [anchor=west] at (8.5,0) {Endemic Security  $\OOT^{\E}$};
