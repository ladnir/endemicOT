...
\begin{figure}[t]
	\framebox{\begin{minipage}{0.95\linewidth}
			...
			\begin{enumerate}
				\item \label{step:senderChosen_step1} The parties invoke $\O^{e}_{k,n}$ where $\rec$ takes the role of the receiver with inputs $\set$. The $\send$ obtains $(m_i)_{i\in[n]}$ and the $\rec$ obtains $(m_i)_{i\in \set}$ from  $\O^{e}_{k,n}$. 
...
\begin{proof}
	...
	Observe that the output of the honest party is identically when $\Adv_1$ interacts with $\rec$ in $\Pi^{\send}_{k,n}$ and when $\Adv_1'$ interacts with $\O^{\send}_{k,n}$.
	
	
	Now let us consider a corrupt receiver $\Adv_2$.   We construct a new $\Adv_2'$ adversary which interacts with the oracle $\O^{\send}_{k,n}$ and produces an identical output distribution to $\Adv_2$. $\Adv_2'$ plays the role of $\O^{e}_{k,n}$ and $\send$ in $\Pi^{\send}_{k,n}$ while running $\Adv_2$. $\Adv_2'$ receives the set $\set\subset[n]$ of size $k$ and the endemic OT strings $m_1,...,m_n$ from $\Adv_1$ in \stepref{step:senderChosen_step1}
\end{proof}0
