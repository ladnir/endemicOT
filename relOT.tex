...
\begin{lemma}
	$\Pi^{\send}_{k,n}$ of \figureref{fig:protoSendOT} realizes the Sender Chosen Ideal OT $\OOT^{\send}$ (\definitionref{def:ot}) with unconditional security.
\end{lemma}
...
\begin{lemma}
	$\Pi^{\rec}_{k,n}$ of \figureref{fig:protoRecvOT} realizes the Receiver Chosen Ideal OT $\OOT^{\rec}$ (\definitionref{def:ot}) with unconditional security.
\end{lemma}
...
\begin{proof}
	...
	Therefore, any distinguishing advantage adversary $\Adv_1$ or $\Adv_2$ has in protocol $\Pi^{\rec}_{k,n}$ implies the same advantage $\Adv_1'$ or $\Adv_2'$ has against the instantiation of $\O^{\rec}_{k,n}$, e.g. $\negl$.
\end{proof}
