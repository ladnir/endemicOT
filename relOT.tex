...
\begin{lemma}\label{lem:EtoU}
	$\Pi^{\U}$ of \figureref{fig:uniformOT} realizes the Uniform Ideal OT $\OOT^{\U}$ (\definitionref{def:ot}) with unconditional security in the $\OOT^\E,\F^{\coin}$ (\definitionref{def:coin}) hybrid.
\end{lemma}
\iffullversion
\begin{proof}
