...
% 	It is important to distinguish that while security will not be violated, replacing a sender or receiver chosen message OT protocol with a uniform message OT protocol may result in the larger protocol aborting due to the distribution of the messages being uniform as opposed to chosen. 
	See  \appendixref{sec:addOT} for a formal proof of \lemmaref{lemma:is_a}.\pe
\end{proof}
\fi
...
\node [anchor=west] at (5.5,5) {Receiver$(\set\subset[n])$:};
...
\node [anchor=west] at (5.5,3.5){$\forall i\in\set:$};
\node [anchor=west] at (5.5,3){$\quad s_i:=r_i\oplus m_i$};

\node [anchor=west] at (0,2.5) {output $(s_i)_{i\in[n]}$};
\node [anchor=west] at (5.5,2.5) {output $(s_i)_{i\in\set}$};
\end{tikzpicture}
}
...
\node [anchor=west] at (5.5,5) {Receiver$(\set,(s_i)_{i\in\set})$:};
...
\node [anchor=west] at (5.5,4.5){$\forall i\in[n]\setminus\set:$};
\node [anchor=west] at (5.5,4){$\quad r_i\leftarrow\bits^\ell$};
\node [anchor=west] at (5.5,3.5){$\forall i\in\set:$};
\node [anchor=west] at (5.5,3){$\quad r_i:=s_i\oplus m_i$};
...
\node [anchor=west] at (5.5,2.8){$\forall i\in\set:$};
\node [anchor=west] at (5.5,2.3){$\quad s_i:=r_i\oplus m_i$};

\node [anchor=west] at (0,1.8) {output $(s_i)_{i\in[n]}$};
\node [anchor=west] at (5.5,1.8) {output $(s_i)_{i\in\set}$};
\end{tikzpicture}
}
% 	\framebox{\begin{minipage}{0.95\linewidth}
...
\node [anchor=west] at (0,5) {Sender:};
\node [anchor=west] at (5.75,5) {Receiver:};
\begin{scope}[shift={(-1.25,0)}]
\draw [thick] (4.75,4.65)--(5.75,4.65)--(5.75,3.65)--(4.75,3.65)--cycle;
...
\node [anchor=west] at (5.75,3.5) {output $(m_i)_{i\in[k]}$};
\end{tikzpicture}
}
% 	\framebox{\begin{minipage}{0.95\linewidth}
