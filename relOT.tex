...
\begin{lemma}\label{lem:OTtoC}
	$\Pi^{\textsf{coin}}$ of \figureref{fig:coinFlip} realizes an ideal coin flipping protocol (\definitionref{def:coin}) with unconditional security  in the $\O^{\U}$ hybrid  (\definitionref{def:ot}).
\end{lemma}
\begin{proof}
