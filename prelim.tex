...
An \emph{ideal $k$-out-of-$n$ oblivious transfer} is an oracle that interacts with two parties, a sender \send and a receiver \rec. It takes a subset  $\set\subseteq[n]$ of size $k$ from \rec. We distinguish four methods for choosing the strings $s_1,...,s_n\in \{0,1\}^\ell$. In all cases, the oracle concludes by sends $(s_i)_{i\in[n]}$ to \send and $(s_i)_{i\in\set}$ to \rec.  The methods are:
\begin{description}
\item[Sender Chosen Ideal OT:] We refer to this oracle as $\OOT^{\send}$. $\send$ chooses $(s_i)_{i\in[n]}$ and sends it to $\OOT^{\send}$. 
\item[Receiver Chosen Ideal OT:] We refer to this oracle as  $\OOT^{\rec}$. \rec chooses $(s_i)_{i\in\set}$ and sends it to  $\OOT^{\rec}$.  $\OOT^{\rec}$ samples the undefined strings $(s_i)_{i\in[n]}\setminus(s_i)_{i\in\set}$ uniformly at random.  
\item[Uniform Ideal OT:] We refer to this oracle as  $\OOT^{u}$. $\OOT^{u}$ samples $(s_i)_{i\in[n]}$ uniformly at random and sends $(s_i)_{i\in[n]}$ to \send and $(s_i)_{i\in\set}$ to \rec.  
\item[Endemic Ideal OT:] We refer to this oracle as  $\OOT^{e}$. If $\send$ is corrupt, $\OOT^{e}$ receives the strings $(s_i)_{i\in[n]}$ from $\send$. Otherwise, if $\rec$ is corrupt, $\OOT^{e}$ receives $(s_i)_{i\in\set}$ from $\rec$. Any undefined $s_i$ for $i\in [n]$ are sampled uniformly at random by $\OOT^{e}$. 
\end{description}
\end{definition}
