...
In our security analysis, the simulator uses aborts. In this case, he will restart the adversary, i.e. rewind the adversary. Hence we only obtain stand-alone security. In Appendix , we show UC security for our two-round construction and in Appendix , we show UC security based on a variant of \DDH of the one-round OT based on the Diffie-Hellman key agreement.
...
\node [anchor=west] at (-0.35,3){$\mes_{\A,j}=r_j\oplus\H((r_\ell)_{\ell\neq j})$};
...
\node [anchor=west] at (4,3){$r_{\i}=\mes_\A\ominus \H((r_j)_{j\neq\i})$};
