\section{From Key Agreement to Oblivious Transfer}
...
\node [anchor=west] at (7,2) {$\key_{\A,\i}:=\UKA.\EK(\tape_\A,\mes_{\B,\i})$};
\node [anchor=west] at (-1,2) {$\quad\key_{\B,j}:=\UKA.\EK(\tape_{\B,j},\mes_{\A,j})$};
\end{tikzpicture}
...
\begin{theorem}
Given a correct and secure [one-round] \UKA scheme, then the $1$ out of $n$ oblivious transfer in Figure~\ref{fig:KAtoOT} [Figure~\ref{fig:oneroundKAtoOT}]  is an Endemic $\OT_{1,n}$ in the programmable random oracle model. 
\end{theorem}
