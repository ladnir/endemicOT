...
\begin{lemma} \label{lem:malRec}
	...
	$$
	where $\Pi^{\textsf{OOS}}$ is the protocol in \definitionref{def:OOS}. All algorithms also receive input $1^\kappa$. 
\end{lemma}
\iffullversion
\input{proofs/attack_BadT0.tex}
\else
\begin{proof}
	See \appendixref{proof:Attack_BadT0}
	\pe
\end{proof}
\fi
...
\begin{lemma} \label{lem:malRec2}
	...
	|\Pr[\D((\send, \Adv)_{\Pi^{\textsf{OOS+}}})=1]-\Pr[\D((\OOT^{\U}, \Adv'))=1]|=1-2^{-\kappa}
	$$
	where $\Pi^{\textsf{OOS+}}$ is the protocol in \definitionref{def:OOS2} and all algorithms additionally receive input $1^\kappa$. 
\end{lemma}
\iffullversion
\input{proofs/attack_Selection.tex}
\else
\begin{proof}
	See \appendixref{proof:Attack_Selection} \pe
\end{proof}
\fi
...
\else
\begin{proof}
	See \appendixref{proof:Attack_AllOnes} \pe
\end{proof}
\fi
...
We now give new security proofs (\lemmaref{lem:ext-E}) of the \cite{C:KelOrsSch15,RSA:OrrOrsSch17} protocols with respect to the $\OOT^\E$ functionality. We then given new 
...
\begin{definition}\label{def:ext_Su_U}
	...
	\begin{enumerate}
		...
		\item In \stepref{step:extInit} of $\Pi^{\textsf{ext-S}u}$, \send samples $k\gets\mathbb{F}^\nc_2$ sends a of $k$ to \rec, i.e. \textcolor{red}{define OR commitment}. 
