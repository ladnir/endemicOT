...
\begin{definition}\label{def:OOS}
	Let $\Pi^{\textsf{OOS}}$ be the protocol of \figureref{fig:otExt} where $\O^{\textsf{chllng}}:=\Pi^{\textsf{chllng}}$ (\figureref{fig:OChallenge}) and $\OOT^*$ is implemented as follows: the parties execute $\OOT^\send$ where both parties provide uniformly sampled inputs, i.e.  \cite[Protocol 2]{OOS16}.
\end{definition}
