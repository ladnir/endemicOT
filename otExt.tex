...
\begin{figure}[h!]
	...
	\node (SC) at (-0.1,3) {Sender Tweakable $\OOT^{\send}$};
	\node (RC) at (-0.35,2) {Receiver Tweakable  $\OOT^{\rec u}$};
	\node (E) at (-0.05,1) {Endemic Security  $\OOT^{\E}$};
	...
	\node (ERC) at (12.38,2) {Receiver Tweakable  $\OOT^{\rec}$};
	\node (EE) at (12.07,1) {Endemic Security  $\OOT^{\E}$};
