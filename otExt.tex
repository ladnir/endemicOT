...
\subsection{Constructions and Security}\label{sec:extSec}
...
\begin{definition}\label{def:ext_R_S}
	Let $\Pi^{\textsf{ext-R}}$ be the protocol of \figureref{fig:otExt} where $\OOT:=\OOT^\rec$, $N=2$, \stepref{step:consistency} is implemented using \cite{C:KelOrsSch15} and the random oracle $\H(i,x)$ required by $\Pi^{\textsf{ext-E}}$ is replaced as follows: after \stepref{step:consistency}, \send samples $k\gets\{0,1\}^{\kappa}$ and sends it to \rec. Both parties define $\H(x)=\pi_k(x)+x$ where $\pi:\{0,1\}^{\kappa} \times \mathbb{F}^\nc_2 \rightarrow\{0,1\}^\kappa$ is an ideal cipher. Note: the $i$ parameter of $\H$ is removed.
\end{definition}
...
%	\begin{claim}[Uniform Messages for Honest \send]
%		The output messages for an honest \send are distributed uniformly by the protocol except with negligible probability. 
%	\end{claim}
%	\begin{proof}
%		First, \equationref{eq:unique} shows that $y_{i,x}:=\tt_i + \bb \odot (\cc_i + \mathcal{C}(map(x)))$ is unique over all $i\in[m],x\in[N]$ with probability at least $1-m^2N'^22^{-\nc}=1-\negl$, where $N'\leq N$ is the maximum number of outputs \send computes for any OT instance.
%		Therefore, the output $\vv_{i,x}=\pi_k(y_{i,x})+y_{i,x}$ of \send is distributed uniformly due to $\pi_k$ being an ideal random permutation. \textcolor{red}{cite DaviesMeyers and say its not invertable.} 
%	\end{proof}
	...
	\begin{proof}
		...
		\begin{enumerate}[leftmargin=1.8cm]
			...
			\item[Hybrid 4.]\label{hybrid:simOutputR} Assuming $\Adv'$ did not abort in \stepref{step:consistency}, for all $w\neq \ww_i$, \Adv has $\negl$ probability of computing $g=\qq_i+\bb\odot \mathcal{C}(w)$. If this was not the case, then \Adv  could compute
			\begin{align*}
			g +\tt_i &=\qq_i+\bb\odot \mathcal{C}(w)+\tt_i\\
			&=\cc_i\odot\bb+\tt_i+\bb\odot \mathcal{C}(w)+\tt_i\\
			&=(\cc_i+\mathcal{C}(w))\odot \bb
...
%\begin{lemma}\label{lem:ext_R_S_honest_sender}
%	$\Pi^{\textsf{ext-R}}$ satisfies Security Against a Malicious Receiver (\definitionref{def:otSec}) with respect to the $\OOT^\U$ oracle.
%\end{lemma}
%\begin{proof}
%	TODO. this is odd.
