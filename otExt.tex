...
\begin{definition}\label{def:ext_R_S}
	Let $\Pi^{\textsf{ext-R}}$ be the protocol of \figureref{fig:otExt} where $\OOT:=\OOT^\rec$ and the random oracle $\H(i,x)$ required by $\Pi^{\textsf{ext-E}}$ is replaced as follows: after \stepref{step:consistency}, \send samples $k\gets\{0,1\}^{\kappa}$ and sends it to \rec. Both parties define $\H(x)=\pi_k(x)+x$ where $\pi:\{0,1\}^{\kappa} \times \mathbb{F}^\nc_2 \rightarrow\{0,1\}^\kappa$ is an ideal cipher. Note: the $i$ parameter of $\H$ is removed.
\end{definition}
...
\begin{definition}\label{def:ext_R_U}
	Let $\Pi^{\textsf{ext-U}}$ be the protocol of \figureref{fig:otExt} where $\OOT:=\OOT^{\U u}$ and the random oracle $\H(i,x)$ required by \figureref{fig:otExt} is replaced as follows:
	\begin{enumerate}
...
\begin{lemma}\label{lem:ext_U_U}
	The $\Pi^{\textsf{ext-U}}$ protocol realizes 1-out-of-$N$ $\OOT^\U$-security and, against a malicious \rec, $\OOT^\U$-security. That is, the input messages of an honest \send are sampled uniformly from $\{0,1\}^\kappa$ by the protocol.
\end{lemma}
\begin{proof}
	\begin{claim}[Malicious Sender Security]\label{claim:ext-U-U-MalSender}
		$\Pi^{\textsf{ext-U}}$ satisfies Security Against a Malicious Sender (\definitionref{def:otSec}) with respect to the $\OOT^\U$ oracle.
	\end{claim}
	\begin{proof}
